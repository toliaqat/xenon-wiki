...
DCP addresses the need to build decentralized control and management plane components that are portable, lightweight (a few hundred bytes overhead per instance), fast, durable and observable. Each component is a tiny "service", listening on HTTP URI, representing some document + behavior. It can serve its own javascript for custom, persona oriented UI. The documents contain references to other services and properties that affect the behavior of the service. A common, replicated, indexed store allows for rich queries across service state, and the ability to <strong>dynamically</strong> compose service views.
...
* **Clustering support** - Groups of nodes can be associated per service, and different level of guarantees can be chosen per service. Node membership is maintained by a scaleable gossip layer (see SWIM paper reference) and replication is done by DCP, on per update or batched mode. 
...
* **Plugable AuthZ / AuthN** - Role base authentication services operating on users/user-groups, resources and verbs enable a simple but powerful authorization model, available across all DCP nodes. Authentication is delegated to third party identity and single-sign-on providers
* Disk limited, not memory limited - DCP implements on demand service instance pause/resume, so not only service state is kept on disk, but the service instance runtime context can be "paged out", on demand, allowing a DCP host to have millions of service instances available, but be limited to just 64MB of heap!
...
The following links should provide context on DCP design and on management functionality build as DCP services

 * [programming model page](dcp-Programming-Model) explains how to author a DCP service, how to select various service options, and how REST actions map to DCP service behavior.
 * [developer guide](dcp-DeveloperGuide) describes how to start writing code in DCP, workflow tools and the key programming interfaces in various languages
...
The service instance is automatically replicated across DCP nodes, and a consensus algorithm guarantees updates and GETs are routed + replicated appropriately.
...
The core principle in DCP is that services have a URI path to identify them, and a document PODO to describe their state. Since every durable service instance is indexed, DCP provides a very powerful discovery and naming service: Discovery is nothing more than a query, using the query task service, and the results are all the services that satisfy the query. So above and beyond a key value store, DCP enables dynamic discovery using natural queries, not just primary keys
...
DCP takes care of managing state, concurrency, metrics, and queueing of requests. Please see the [programming model](dcp-Programming-Model) for details.
...
Note that state version tracking can be bounded per service to last N versions or a storage quota. DCP also tracks per service/document expiration.

# Decentralized design
The decentralized framework is an active-active system. DCP uses consistent hashing and state replication to achieve both load balancing of "work" and high availability. Depending on service options different guarantees are given to a service instance.
...
DCP implements a [pause/resume](servicePauseResume) scheme so even the small runtime overhead of a service instance (less than 500 bytes) gets serialized and indexed on disk, in the DCP blob index. This allows DCP to be limited by disk size, per node, not available memory.
